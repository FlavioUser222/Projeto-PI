...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (video) {
        campos.push(`video = $${indice++}`);
        valores.push(novoVideo);
      }
      if (thumbnail) {
        campos.push(`thumbnail = $${indice++}`);
        valores.push(novoThumbnail);
      }
      ...
      valores.push(id);
      const query = `UPDATE videos SET ${campos.join(', ')} WHERE id = $${indice}`;
